  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 px-[5vw] h-[52px] flex items-center justify-between transition-all duration-300 ${
      ...
      <ul className="hidden md:flex gap-10 list-none">
        <li><Link to="/#como-funciona" className="text-[var(--muted)] no-underline text-xs tracking-widest hover:text-[var(--foreground)] transition-colors">como funciona</Link></li>
        <li><Link to="/#capacidades" className="text-[var(--muted)] no-underline text-xs tracking-widest hover:text-[var(--foreground)] transition-colors">tecnologia</Link></li>
        <li><Link to="/#ecossistema" className="text-[var(--muted)] no-underline text-xs tracking-widest hover:text-[var(--foreground)] transition-colors">ecossistema</Link></li>
        <li><Link to="/quem-somos" className="text-[var(--muted)] no-underline text-xs tracking-widest hover:text-[var(--foreground)] transition-colors">quem somos</Link></li>
        <li><Link to="/blog" className="text-[var(--muted)] no-underline text-xs tracking-widest hover:text-[var(--foreground)] transition-colors">blog</Link></li>
      </ul>
      ...
      {!isHome ? (
        ...
        <button 
          onClick={() => openLeadModal('Organizar meus dados')}
        ...
        >
          organizar meus dados <ChevronRight size={14} />
        </button>
      )}
    </nav>
  );
    ...
    <footer className="w-full bg-[#f3f4f9] text-gray-600 pt-16 pb-8 px-[5vw] border-t border-gray-200/60">
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-8">
        ...
        <div className="flex flex-col space-y-4">
          ...
          <div className="space-y-1 text-sm text-gray-500 leading-relaxed font-light font-sans">
            <p>Sem dados, tudo é palpite.</p>
            <p>Sem inteligência, os dados não têm valor.</p>
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto mt-16 pt-8 border-t border-gray-200/60 text-center font-sans">
        <p className="text-xs text-gray-400 font-light">
          &copy; 2026 boo. Todos os direitos reservados.
        </p>
      </div>
    </footer>
  );
        ...
        >
          <div className="flex items-center gap-2.5">
            <p className="text-[11px] text-slate-500 leading-tight font-light flex-1">
              Usamos cookies conforme a LGPD. Veja a nossa <Link to="/politica-de-privacidade" className="text-slate-900 hover:text-[#46AAFF] transition-colors font-normal no-underline border-b border-slate-300">Política de Privacidade</Link>.
            </p>
            <button
              onClick={handleAccept}
              className="px-2.5 py-1 bg-slate-950 hover:bg-slate-850 text-white rounded-md text-[10px] font-semibold uppercase tracking-wider transition-colors shrink-0"
            >
              Ok
            </button>
          </div>
